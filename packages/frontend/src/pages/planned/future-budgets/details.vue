...
const activeTab = ref<'statistics' | 'transactions'>('statistics');
const tabItems = [
...
const categoryBreakdown = computed(() => {
  ...
  for (const item of data.value?.occurrences ?? []) {
...
const maxCategoryAmount = computed(() => Math.max(1, ...categoryBreakdown.value.map((row) => row.amount)));
</script>
  ...
  <div v-else-if="data" class="mx-auto max-w-6xl space-y-6">
    ...
    <template v-if="activeTab === 'statistics'">
      ...
      <section class="rounded-lg border p-5">
        ...
        <p class="text-muted-foreground mb-4 text-sm">Projected expenses across this plan's date range.</p>
        <div v-if="categoryBreakdown.length" class="space-y-3">
          <div v-for="row in categoryBreakdown" :key="row.label" class="space-y-1">
            <div class="flex items-center justify-between text-sm">
              <span>{{ row.label }}</span>
              <span class="text-app-expense-color font-medium">{{ currency(row.amount) }}</span>
            </div>
            <div class="bg-muted h-2 overflow-hidden rounded-full">
              <div
                class="bg-app-expense-color h-full rounded-full"
                :style="{ width: `${(row.amount / maxCategoryAmount) * 100}%` }"
              />
            </div>
          </div>
        </div>
        <p v-else class="text-muted-foreground text-sm">No projected expenses yet.</p>
      </section>
    </template>
